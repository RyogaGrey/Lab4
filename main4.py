...
def main():
    ...
    parser.add_argument('--log_level', type=str, help="Уровень логирования (DEBUG, INFO, WARNING, ERROR)", default="INFO")
    parser.add_argument('--log_file', type=str, help="Файл для логирования (по умолчанию вывод в консоль)", default=None)

    args = parser.parse_args()

    # Настройка логирования
    if args.log_file:
        logging.basicConfig(filename=args.log_file, level=args.log_level.upper(), format='%(asctime)s - %(levelname)s - %(message)s')
    else:
